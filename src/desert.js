...
export function setupDesertScene() {
  ...
  let cactusGeo = new BoxGeometry(0, 0, 0);
  ...
  (async function() {
    ...
    let textures = {
      ...
      grass: await new TextureLoader().loadAsync("assets/sand.jpg"),
      ...
      cactus: await new TextureLoader().loadAsync("assets/cactus.png"),
    };
    ...
    let cactusMesh = hexMesh(cactusGeo, textures.cactus);
    scene.add(stoneMesh, dirtMesh, dirt2Mesh, sandMesh, grassMesh, cactusMesh);
  ...
  cactusGeo = new BoxGeometry(0,0,0);
  ...
  function hex(height, position) {
    ...
    if(height > STONE_HEIGHT) {
      ...
      if(Math.random() > 0.5) {
        cactusGeo = mergeBufferGeometries([cactusGeo, cactus(height, position)]);
      }
  ...
  function cactus(height, position) {
    const cactusHeight = Math.random() * 1 + 1.5;
  
    // Create cactus body
    const bodyGeo = new CylinderGeometry(0.2, 0.2, cactusHeight * 0.7, 8);
    bodyGeo.translate(position.x, height + cactusHeight * 0.35, position.y);
  
    // Create cactus arms
    const arm1Geo = new CylinderGeometry(0.1, 0.05, cactusHeight * 0.8, 4);
    arm1Geo.translate(position.x - 0.25, height + cactusHeight * 0.65, position.y);
  
    const arm2Geo = new CylinderGeometry(0.1, 0.05, cactusHeight * 0.8, 4);
    arm2Geo.translate(position.x + 0.25, height + cactusHeight * 0.65, position.y);
  
    return mergeBufferGeometries([bodyGeo, arm1Geo, arm2Geo]);
  }
